...
namespace bcrwss
{
    public partial class fMain : Form
    {
        ...
        private void OnWebsocketReceive(string json)
        {
            try
            {
                ...
                else if (json.IndexOf("kickout") > -1)
                {
                    ...
                    this.toolStripStatusMain.Text = "Error";
                    wsClient.Dispose();
                }
            }
            catch(Exception ex)
        ...
        private void tMetricPing_Tick(object sender, EventArgs e)
        {
            bHandler.RandomSwitchLayoutEvo();
            if (IsWebSocketError)
            {
                StartSessionWithoutLogin();
            }
        }

        private void StartSessionWithoutLogin()
        {
            StopCurrentSession();
            DoGoToEvoLobby();
            UpdateLobbyInfo();
        ...
        private void tRefreshSession_Tick(object sender, EventArgs e)
        {
            StartSessionWithoutLogin();
            tMetricPing.Stop();
            tMetricPing.Start();
        ...
        {
            cBrowser.Load("https://qq8788viet.com/vi-VN/Home");
        ...
        private void DoGoToEvoLobby()
        {
            ...
            var task = Task.Run(async () =>
            {
                while (!bHandler.IsEvoBrowserReady())
                {
                    await Task.Delay(1000);
                }
                await Task.Delay(1000);
        ...
        private void StopCurrentSession()
        {
            ...
            this.toolStripStatusMain.Text = "Stopped";
        }
